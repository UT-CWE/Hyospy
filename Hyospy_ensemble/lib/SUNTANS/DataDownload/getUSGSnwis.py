...
def readUSGStxt(stationid,starttime,endtime):
    ...
    target_url = 'http://nwis.waterdata.usgs.gov/tx/nwis/uv?cb_00060=on&format=rdb&site_no=%s&period=&begin_date=%s&end_date=%s'%(stationid,starttime,endtime)            
    ...
    for s in f:
	    ...
	    time.append(datetime.strptime(line[2]+' '+line[3],'%Y-%m-%d %H:%M'))
	    #if type(line[5]) == str:
	    #    discharge.append(0*scale_fac)
	    #else:
            #    discharge.append(float(line[5])*scale_fac)
	    if line[5] == '' or line[5] == 'Eqp':
		discharge.append(0*scale_fac)
	    else: 
		discharge.append(string.atof(line[5])*scale_fac)
